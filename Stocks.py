...
if mode == "7" or mode0 == "7":
  ...
  for i in tickers:
    for j in i:
      os.system('clear')
      print(fcount, "/", len(tickers[0])+len(tickers[1])+len(tickers[2])+len(tickers[3]), "   ", round((fcount/(len(tickers[0])+len(tickers[1])+len(tickers[2])+len(tickers[3])))*100, 2), "%")
      tick = yf.Ticker(j)
      dyb = [str(int(date.today().strftime("%Y")) - 1), str(date.today().strftime("%m")), str(date.today().strftime("%d"))]
      dy = "-".join(dyb)
      try:
...
if mode == "8" or mode0 == "8":
  ...
  for i in tickers:
    for j in i:
      os.system('clear')
      print(fcount, "/", len(tickers[0]), round((fcount/len(tickers[0]))*100, 2), "%")
      tick = yf.Ticker(j)
      dyb = [str(int(date.today().strftime("%Y")) - 1), str(date.today().strftime("%m")), str(date.today().strftime("%d"))]
      dy = "-".join(dyb)
      try:
